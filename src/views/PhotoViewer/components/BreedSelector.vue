<template>
  <div class="BreedSelector">
    ...
    <select id="breedSelect" value="{{ selectedBreed || undefined }}" @change="selectedBreedChange">
      ...
      <option v-for="breed of breedList" :key="breed.id" :value="breed.id" :name="breed.name">{{ breed.name }}</option>
    </select>
...
<script setup lang="ts">
  ...
  const breedList = computed(() => breedStore.breedList);
  const isLoading = computed(() => breedStore.isLoading);
  const selectedBreed = computed(() => breedStore.selectedBreed);

  onMounted(() => breedStore.fetchBreedList());

  const selectedBreedChange = (event: Event): void => {
    breedStore.selectBreed((event.target as HTMLSelectElement).value);
  };
</script>
